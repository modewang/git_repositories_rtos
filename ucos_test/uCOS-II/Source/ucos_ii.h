...
// task的extension功能相关特性
    ...
    INT8U            OSTCBX; // 由优先级计算出的组中的bit位置  /* Bit position in group  corresponding to task priority   */
    INT8U            OSTCBY; // 由优先级计算出的就绪表下标     /* Index into ready table corresponding to task priority   */
#if OS_LOWEST_PRIO <= 63
...
OS_EXT  INT32U            OSIdleCtrRun; // 1秒前空闲任务计数器值  /* Val. reached by idle ctr at run time in 1 sec.  */
OS_EXT  BOOLEAN           OSStatRdy;    // 是否执行统计任务的标识  /* Flag indicating that the statistic task is rdy  */
...
OS_EXT  OS_TCB           *OSTCBFreeList;  // 指向free list单链表的头   /* Pointer to list of free TCBs             */
